...
function getToken(){
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL,"https://ecng-identity.azurewebsites.net/connect/token");
...
function createPaymentIntent($amount){
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL,"https://ecng-transactions.azurewebsites.net/api/paymentIntent");
	curl_setopt($ch, CURLOPT_POST, 1);
	...
	$data = array('currency'=>'ILS',
				  'dueDate'=>NULL,
				  'dealDetails'=> array(
					'consumerEmail' => 'kanivetsig@mydigicode.com',
...
function getTransactionById($transactionID){
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL,"https://ecng-transactions.azurewebsites.net/api/transactions/".$transactionID);
	//Remove for production code. ONLY FOR TEST PURPOSES
	curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
